...
public class BrassGearboxVisual extends KineticBlockEntityVisual<BrassGearboxBlockEntity> {

    protected final EnumMap<Direction, RotatingInstance> keys = new EnumMap<>(Direction.class);
    protected Direction sourceFacing;

    public BrassGearboxVisual(VisualizationContext context, BrassGearboxBlockEntity blockEntity, float partialTick) {
    ...
    private float getSpeed(Direction direction) {
        ...
        if (speed != 0 && sourceFacing != null) {
            speed *= BrassGearboxBlockEntity.getRotationSpeedModifier(direction, sourceFacing, blockState);
        }
        return speed;
    }
